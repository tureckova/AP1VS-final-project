...
import unittest
from main import get_file_choice, get_user_input, choices
...
class TestCountLetters(unittest.TestCase):
    ...
    class TestChoices(unittest.TestCase):
        ...
        def test_file_choice(self):
            """
            Kontroluje, zda funkce get_file_choice() vrací "test_text".

            :param self:
            """
            self.assertEqual(get_file_choice(), "test_text")

        def test_user_input(self):
            """
            Kontroluje, zda funkce get_user_input() vrací "test_text".

            :param self:
            """
            self.assertEqual(get_user_input(), "test_text")

        def test_choices(self):
            """
            Unit test pro funkci choices().

            Kontroluje, zda funkce choices() vrátí "test_text",
            pokud je jako argument zadáno "1" nebo "2".

            :param self:
            """
            self.assertEqual(choices("1"), "test_text")
            self.assertEqual(choices("2"), "test_text")
